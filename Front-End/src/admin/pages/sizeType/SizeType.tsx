...
const SizeType: React.FC = () => {
  ...
  const handleSubmit = async () => {
    if (!inputData) {
      return (
        setBackEndError(""),
        setError("Size Type is Required")
      );
    }
    const data = {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/sizehead", data);
  ...
  const deleteItem = async (_id: string) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:5000/api/sizehead/${_id}`);
  ...
  const fetchUpdateData = async (_id?: string) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/sizehead/${_id}`);
  ...
  const handleUpdate = async () => {
    try {
      const afterUpdate = await axios.put(`http://localhost:5000/api/sizehead/${clickUpdateId}`, { sizeHeadName: inputData });
      setInputData('');
      setClickUpdateId('');
      setBackEndError("")
    } catch (error: any) {
        ...
        <table className={styles.districtTable}>
          ...
          <tbody>
            {alldata.length > 0 ? (
              ...
              <tr>
                <td colSpan={3} className={styles.noData}>No Size Types</td>
              </tr>
            )}
          </tbody>
        </table>
